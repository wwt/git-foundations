...
5. Issue commands to:
    ...
    !!! tip
        If you happen to end up in the Docker Container VIM editor, press the ++esc++ key, type ++colon++, ++q++, ++exclam++, and press ++enter++ or ++"Return"++.
...
8. View the status of your local Git repository with the following command:
...
10. Sometimes it is necessary to roll-back to a previous repository commit or to understand the history of changes to a repository (who did what to which files).  You can review the complete commit history for a repository with the `git log` command.
